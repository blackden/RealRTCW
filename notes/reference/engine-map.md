...
- **`refexport_t` adapter — `code/client/cl_refvulkan_export.c` (M5 closure 2026-06-12).**
  Mirror of the refImport adapter, opposite direction (renderer→engine). Vendored renderervk fills BIG `refexport_t` (38 slots, Q3e layout) via `GetRefAPI`; engine `re` global is SMALL (36 slots, RTCW layout) — slot offsets diverge starting at position 5. Without translation, `re.RegisterShader("white")` (engine call) misdispatches to `RE_LoadWorldMap` (renderer slot at same byte offset), crashes immediately. `CL_BuildVulkanRefExport()` (in `cl_refvulkan_export.c`) builds a SMALL `refexport_t` slot-by-slot using accessors + thunks defined in `cl_refvulkan.c` (BIG-side TU); two TUs needed because SMALL and BIG `tr_public.h` share `__TR_TYPES_H` guard and can't coexist. Wire-in at `cl_main.c:3523` between `GetRefAPI` return and `re = *ret`, under `#ifdef BUILD_RENDERER_VULKAN`. 36/36 slots: 21 Group A identity (`VK_RE_GET(name)` macro accessors), 7 Group B sig-translating wrappers (Shutdown qboolean→refShutdownCode_t, AddRefEntityToScene 1→2 args, AddPolyToScene 3→4 args, AddLightToScene drop overdraw, LerpTag refent→model+frames reshape, DrawStretchRaw and UploadCinematic const-strip), 8 Group C RTCW-only tripwire stubs (log-once-per-slot, no-op after). Decision: `notes/decisions/2026-06-12-m5-refexport-translator-landed.md`.
