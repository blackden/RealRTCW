...
// RealRTCW shim: Quake3e renderervk declares a few engine-side callbacks and
// macros that don't exist in the older RealRTCW (~iortcw 2014) engine. We
// stub them out so this header still type-checks. Renderer code never calls
// these on RealRTCW (they're only listed in the vtable for source parity);
// the engine never wires real implementations to those vtable slots either.
